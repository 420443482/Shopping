...
function open_window(title, url) {
    ...
    if (!bStop) {
        ...
        showBox.find('iframe').load(function () {
            ...
            nav_title += "<span class='c-gray en'>&gt;</span>"+title;
            $(showBox.find('iframe')[0].contentWindow.document.querySelector('#nav-title')).append(nav_title)
...
function ajax_progress_ext(ret){
    if( 0 === ret.code ) {
        layer.open({
 ...
 * @param shade 是否遮罩
 */
function ajax_api(url, data, callback,is_load) {
    if( is_load ) {
 ...
 * @param fn 回调函数
 */
function save_status(obj, id, url,msg,type, fn) {
    _save(obj, id, url, msg,type, fn);
}
/**
 * 从回收站恢复操作
 * @param obj this
...
function _save(obj, id, url, msg,type, fn) {
    _del_recycle(obj, id, url, msg, type, fn)
}
